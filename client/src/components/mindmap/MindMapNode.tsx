...
export default function MindMapNode({ d3Node, isSelected, isDragTarget, hasChildren, onMouseDown }: MindMapNodeProps) {
  const { selectNode, updateNode, deleteNode, toggleCollapse, copyNode, pasteNode, selectedNodeId, copiedNode, currentMap } =
    useMindMapStore();
    ...
    <>
      ...
      {ctxMenu && (
        <ContextMenu
          ...
          onEdit={() => { setEditText(node.title); setEditing(true); }}
          onDelete={() => deleteNode(node.id)}
          onCopy={() => copyNode(node.id)}
          onPaste={() => pasteNode(node.id)}
