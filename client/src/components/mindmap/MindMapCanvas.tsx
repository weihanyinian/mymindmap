...
export default function MindMapCanvas() {
  ...
  // Compute D3 layout whenever the tree changes
  useEffect(() => {
    if (!currentMap) {
      setLayoutRoot(null);
      return;
  ...
  useEffect(() => {
    ...
    const z = setupZoom(svg, zoomGroup, () => {});

    return () => {
      svg.removeEventListener('wheel', z as unknown as EventListener);
      svg.removeEventListener('mousedown', z as unknown as EventListener);
    };
  }, []);

  const handleSvgClick = useCallback(
    (e: React.MouseEvent) => {
      if (e.target === svgRef.current || e.target === zoomGroupRef.current) {
        selectNode(null);
      }
    },
    [selectNode]
  ...
  if (!currentMap || !layoutRoot) {
    return (
      <div className="flex-1 flex items-center justify-center bg-gray-50">
        <p className="text-gray-400">No mind map loaded</p>
      </div>
    );
  }

  // Collect all visible nodes and connections for rendering
  ...
  function collectNodes(d3Node: LayoutNode, parentId: string | null) {
    ...
    if (parentId && d3Node.parent) {
      connections.push({ from: d3Node.parent as LayoutNode, to: d3Node });
    }
    if (d3Node.children) {
      for (const child of d3Node.children) {
        collectNodes(child as LayoutNode, d3Node.data.id);
      }
    }
  }
  ...
  return (
    <div className="flex-1 bg-gray-50 overflow-hidden relative">
      ...
      >
        ...
        <g ref={zoomGroupRef}>
          ...
          {connections.map(({ from, to }) => (
            <ConnectionLine
              key={`conn-${from.data.id}-${to.data.id}`}
              from={from}
              to={to}
              nodeWidth={140}
      ...
      <div className="absolute top-2 left-2 text-xs text-gray-400 bg-white/80 rounded px-2 py-1 z-10">
        {currentMap.title}
      </div>
    </div>
  );
